...
def whole_wide_dataset(weekly_returns, start_train_date, end_test_date):
    whole_dataset = weekly_returns[(weekly_returns.index >= start_train_date) & (weekly_returns.index <= end_test_date)].copy()
    return whole_dataset


@pytest.fixture(scope="session")
def train_wide_dataset(weekly_returns, start_train_date, end_train_date):
    train_dataset = weekly_returns[(weekly_returns.index >= start_train_date) & (weekly_returns.index <= end_train_date)].copy()
...
def test_narrow_dataset(weekly_returns, start_test_date, end_test_date, subset_of_assets):
    test_dataset = weekly_returns[(weekly_returns.index > start_test_date) & (weekly_returns.index <= end_test_date)].copy()
