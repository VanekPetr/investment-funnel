...
TDF_data = pd.read_excel(
    "C:/Users/MIBEM/OneDrive - Ørsted/Desktop/Thesis/saved_data/dataForAnnualisedTDFvols.xlsx",
    sheet_name="data_overview",
)
ann_vols = pd.read_excel(
    "C:/Users/MIBEM/OneDrive - Ørsted/Desktop/Thesis/saved_data/dataForAnnualisedTDFvols.xlsx",
    sheet_name="campbell",
)
...
fig = make_subplots(
    rows=3, cols=1, shared_xaxes=False, vertical_spacing=0.03
)  # Reduced spacing
...
for i in range(1, 4):
    fig.update_yaxes(
        title_text="Allocation Percentage" if i == 2 else "",
        showgrid=True,
        showline=True,
    ...
    fig.update_xaxes(
        title_text="Years to target date" if i == 3 else "",
        showgrid=True,
        showline=True,
        gridcolor="rgba(211, 211, 211, 0.5)",
        row=i,
...
for i, label in enumerate(subplot_labels, start=1):
    fig.add_annotation(
        ...
        xref="paper",
        x=-0.1,  # Positioning the label to the left of the plot
...
fig.add_trace(
    go.Scatter(
        x=df["Year"],
        y=df["Timevariant annualised stock vol"],
        name="Stock Vol",
        mode="lines",
...
fig.add_trace(
    go.Scatter(
        x=df["Year"],
        y=df["Timevariant annualised bond vol"],
        name="Bond Vol",
        mode="lines",
...
fig.update_xaxes(
    title_text="Years to target date",
    showgrid=False,
    showline=True,
...
fig.update_yaxes(
    ...
    range=[
        0,
        max(
            df["Timevariant annualised stock vol"].max(),
            df["Timevariant annualised bond vol"].max(),
        )
        * 1.1,
    ],
)
...
fig.update_yaxes(
    ...
    title_font=dict(size=15),
    tickformat=".2f",
    range=[0, df["Stock/Bond correlation"].max() * 1.1],
)
...
fig.add_trace(
    go.Scatter(
        x=df["Year"],
        y=df_riskprofiles["Vanguard"],
        name="Vanguard Risk Profile",
        mode="lines",
        line=dict(width=2.5, color=colors[0]),
    )
)
...
fig.add_trace(
    go.Scatter(
        x=df["Year"],
        y=df_riskprofiles["Fidelity"],
        name="Fidelity Risk Profile",
        mode="lines",
        line=dict(width=2.5, color=colors[1]),
    )
)
...
fig.add_trace(
    go.Scatter(
        x=df["Year"],
        y=df_riskprofiles["T.RowePrice"],
        name="T.RowePrice Risk Profile",
        mode="lines",
        line=dict(width=2.5, color=colors[2]),
    )
)
...
fig.add_trace(
    go.Scatter(
        x=df_riskprofiles["Year"],
        y=df_riskprofiles["Risk Budget"],
        name="Risk Budget for target date fund",
        mode="lines",
        line=dict(width=2.5, color=colors[0]),
    )
)
...
fig.update_yaxes(
    ...
    tickformat=".0%",
    title_font=dict(size=15),
    range=[0.01, df_riskprofiles["Risk Budget"].max() * 1.2],
)
