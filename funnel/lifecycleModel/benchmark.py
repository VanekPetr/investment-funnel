...
def create_asset_class_df(tickers):
    ...
    '''
    asset_classes = ['risky', 'risky', 'risky', 'risky', 'non_risky', 'non_risky', 'risky', 'risky', 'risky', 'risky',
                     'non_risky', 'risky', 'non_risky', 'non_risky', 'non_risky', 'non_risky', 'non_risky', 'non_risky',
                     'non_risky', 'risky', 'risky', 'risky', 'risky', 'risky', 'risky', 'risky', 'risky', 'risky', 'risky',
                     'risky', 'risky', 'risky', 'risky', 'risky', 'non_risky']
    ...
    if 'Cash' not in asset_tickers:
        ...
        asset_classes.append('non_risky')  # Assuming "Cash" is classified as 'non_risky'

    # Create the DataFrame
    asset_class_df = pd.DataFrame(asset_classes, index=asset_tickers, columns=['Asset Class'])
...
def calculate_target_allocation(annual_alloc_targets, tickers):
    ...
    if len(tickers) != 35:
        logger.debug(f'Right now we work with a special amount of Minimum Spanning Tree runs. Please choose 5 MST '
                     f'runs such that we work with the 35 assets that we have already determined asset class for. '
                     f'If we still see an error, there might have been changes to the ETF data. Please go to the '
                     f'benchmark module and choose the tickers as the subset of data.')
        return  # Stops the function and exits early
    logger.debug(f'Adding allocation targets from the naive 1/N bond/stock allocation strategy, '
                 f'that starts with 100% invested in stocks and glides towards 0% invested in stocks.')
    ...
    allocation_df = pd.DataFrame(0, index=annual_alloc_targets.index, columns=asset_class.index)

    # Classify assets into risky and non_risky categories
    risky_assets = asset_class.index[asset_class['Asset Class'] == 'risky'].tolist()
    non_risky_assets = asset_class.index[asset_class['Asset Class'] == 'non_risky'].tolist()
    ...
    for year in annual_alloc_targets.index:
        allocation_df.loc[year, risky_assets] = annual_alloc_targets.loc[year, 0] / num_risky if num_risky else 0
        allocation_df.loc[year, non_risky_assets] = (1 - annual_alloc_targets.loc[
            year, 0]) / num_non_risky if num_non_risky else 0
