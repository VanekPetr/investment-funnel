...
from funnel.lifecycleModel.MVOlifecycleModel import get_port_allocations, riskadjust_model_scen
from funnel.lifecycleModel.ScenarioGeneration import ScenarioGenerator, MomentGenerator
...
ticker_name_df = pd.DataFrame({'Ticker': [pair[0] for pair in weekly_returns.columns.values],
                               'Name': [pair[1] for pair in weekly_returns.columns.values]})

withdrawal_lst = [27200, 33900, 37200, 39800, 42300, 44500, 46400, 48200, 50000, 51800,
                  53600, 46100, 47000, 48100, 49200, 50900, 52200, 53500, 54800, 56000,
                  57200, 58400, 65500, 89900, 90800, 91800, 92800, 93700]
...
class TradeBot(object):
    ...
    def __plot_portfolio_densities(
        ...
        for label, df in portfolio_performance_dict.items():
            ...
            fig.add_trace(go.Scatter(
                ...
                line=dict(color=next(color_cycle))  # Assign color from OrstedColors
            ))

        # Add a dashed vertical line at x=0
        fig.add_shape(
            type="line",
            x0=0, y0=0, x1=0, y1=max([max(density) for _, df in portfolio_performance_dict.items()]),
    ...
    def scenario_analysis(
        ...
        if scenarios_type == 'MonteCarlo':
            scenarios = sg.MC_simulation_annual_from_weekly(weekly_mu=mu_weekly,
                                                            weekly_sigma=sigma_weekly,
                                                            n_simulations=n_simulations,
                                                            n_years=n_periods)
        elif scenarios_type == 'FHS':
            scenarios = sg.FHS(data=self.weeklyReturns[subset_of_assets],
                               n_simulations=n_simulations,
                               n_years=n_periods)
        else:
            logger.debug(
                f"Currently we only simulate returns with the Monte Carlo method or FHS.")

        # ------------------------------- TARGETS GENERATION -------------------------------
        if model == 'Markowitz model':
            # ------------------------------- Risk Target Generation -------------------------------
            if risk_test == 'simpleLinear':
                targets_risk = pd.DataFrame(np.linspace(0.2, 0.05, n_periods), columns=['Linear Glide Path'])
                ...
                generator = RiskCurveGenerator(
                    periods=n_periods,
                    risk_floor=0.02,
                    std_devs={'Risk Class 3': 0.05, 'Risk Class 4': 0.10,
                              'Risk Class 5': 0.15, 'Risk Class 6': 0.25, 'Risk Class 7': 0.30}
                )

                targets_risk = generator.generate_curves()
                targets_risk = generator.filter_columns_by_risk_class(targets_risk, risk_class)

            else:
                logger.debug(
                    f"We only work with simpleLinear or investmentFunnel risk curves.")

            # ------------------------------- Allocation Target Generation -------------------------------
            allocation_targets = {}
            for r in targets_risk.columns:
                targets = get_port_allocations(mu_lst=mu,
                                               sigma_lst=sigma,
            ...
            if compare_with_naive:
                ...
                naive_allocation = calculate_target_allocation(class_alloc_targets, subset_of_assets)
                allocation_targets[f'1/N bond/stock portfolio'] = naive_allocation
        else:
            logger.debug(
                f"Currently we only have the Markowitz model approach.")
        ...
        if model == 'Markowitz model':
            for key, df in allocation_targets.items():
                logger.info(f'Optimizing portfolio for {key} over {n_simulations} scenarios. An info message will '
                ...
                portfolio_df['Terminal Wealth'] = pd.to_numeric(portfolio_df['Terminal Wealth'],
                                                                errors='coerce')
        ...
        # ------------------------------- PLOTTING -------------------------------
        fig_performance = self.__plot_portfolio_densities(portfolio_performance_dict=terminal_wealth_dict)
...
if __name__ == "__main__":
    ...
    algo = TradeBot()
    mst_ISIN_all, mstdf_all = algo.mst(start_date="2000-01-01",
                                       end_date="2024-01-01",
                                       n_mst_runs=4)
